  ...
  <script>
    ...
    const I18N = {
      zh: {
        ...
        automatic_payroll:'自动工资', legacy_salary_requests:'旧工资申请（历史）', legacy_salary_records:'工资记录（历史）', payroll_detail:'自动工资详情', payment_proofs:'付款凭证', view_detail:'查看详情', view_proof:'查看凭证', save_split:'保存付款拆分', usdt_qr_provided:'USDT 二维码：已提供', usdt_qr_missing:'USDT 二维码：未提供', view_usdt_qr:'查看 USDT 二维码', payroll_id:'工资 ID', scheduled_date:'发薪日期', cutoff_at:'截止时间', amount_snapshot_micros:'固定工资', bank_micros:'银行卡金额', usdt_micros:'USDT 金额', cash_micros:'现金金额', proof_count:'凭证数', email_status:'邮件状态', email_attempt_count:'邮件尝试次数', email_last_error:'邮件错误', current_admin_id:'当前管理员', method:'方式', sort_order:'序号', size_bytes:'文件大小', uploaded_by:'上传人', uploaded_at:'上传时间', superseded_at:'已替换时间',
      ...
      ru: {
        ...
        automatic_payroll:'Автоматическая зарплата', legacy_salary_requests:'Старые запросы зарплаты (история)', legacy_salary_records:'Записи зарплаты (история)', payroll_detail:'Детали автоматической зарплаты', payment_proofs:'Подтверждения оплаты', view_detail:'Открыть детали', view_proof:'Открыть подтверждение', save_split:'Сохранить разделение', usdt_qr_provided:'QR-код USDT: предоставлен', usdt_qr_missing:'QR-код USDT: не предоставлен', view_usdt_qr:'Открыть QR-код USDT', payroll_id:'ID зарплаты', scheduled_date:'Дата выплаты', cutoff_at:'Время отсечения', amount_snapshot_micros:'Фиксированная зарплата', bank_micros:'Сумма банка', usdt_micros:'Сумма USDT', cash_micros:'Наличные', proof_count:'Подтверждения', email_status:'Статус email', email_attempt_count:'Попытки email', email_last_error:'Ошибка email', current_admin_id:'Текущий администратор', method:'Способ', sort_order:'Порядок', size_bytes:'Размер файла', uploaded_by:'Загрузил', uploaded_at:'Время загрузки', superseded_at:'Время замены',
    ...
    async function renderSalary() {
      ...
      document.querySelectorAll('[data-payroll-detail]').forEach((button) => {
        button.onclick = () => withBusy(button, async () => {
          ...
          const usdtQrControl = payroll.has_usdt_qr
            ? '<div class="row"><span>' + L('usdt_qr_provided') + '</span><a target="_blank" rel="noopener" href="' + esc(payroll.usdt_qr_url) + '">' + L('view_usdt_qr') + '</a></div>'
            : '<div class="row"><span>' + L('usdt_qr_missing') + '</span></div>';
